...
```bash
src/vendor/bin/sail artisan horizon:install
```
...
1. Run `src/vendor/bin/sail artisan horizon` and visit `/horizon`
...
- The metrics dashboard stays blank until `horizon:snapshot` is scheduled. Running `src/vendor/bin/sail artisan horizon` alone does not populate metrics.
