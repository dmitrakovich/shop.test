...
@section('content')
<script src="https://api-maps.yandex.ru/2.1/?apikey=f8c761a5-ac2c-4665-a146-d4c23407c140&lang=ru_RU" type="text/javascript"></script>
<div class="col-12 static-page">
    <nav>
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            @foreach($cities as $city)
            <a class="nav-item nav-link @if($loop->first) active @endif" id="js-cityTab-{{ $city->id }}-tab" data-toggle="tab" href="#js-cityTab-{{ $city->id }}" role="tab" aria-controls="nav-home" aria-selected="true">{{ $city->name }}</a>
            @endforeach
        </div>
    </nav>
    <div class="tab-content" id="nav-tabContent">
        @foreach($cities as $city)
        <div class="tab-pane fade show @if($loop->first) active @endif" id="js-cityTab-{{ $city->id }}" role="tabpanel" aria-labelledby="nav-home-tab">
            <div class="row mt-4">
                ...
                <div class="col-md-4 mb-5">
                    ...
                    <div class="col-12 @if(!$loop->last) border-bottom @endif mb-2">
                        <p>
                            <b>{{ $shop->name }}</b>
                            <a href="tel:{{ $shop->phone }}" class="float-right text-decoration-underline">
                                <b>Позвонить</b>
                            </a><br>
                            {{ $shop->address }}
                        </p>
                        <p>{{ $shop->worktime }}</p>
                        <p>
                            <a data-fancybox data-src="#js-shopPhotos-{{ $shop->id }}" href="javascript:;" class="text-primary cursor-pointer">
                                смотреть фото
                            </a>
                        </p>
                    </div>
