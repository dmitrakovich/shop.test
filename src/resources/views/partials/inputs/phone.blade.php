...
<div class="js-phone-component">
    <div class="input-group">
        <div class="input-group-prepend">
            <button class="btn btn-outline-secondary dropdown-toggle py-0 px-3 js-phone-select-country"
                type="button" data-toggle="dropdown" aria-expanded="false" @disabled($readonly)>
            ...
            <div class="dropdown-menu h-auto overflow-auto" style="max-height: 200px;">
                @foreach ($countries as $country)
                    <a class="dropdown-item px-3 js-phone-country" href="{{ $country->code }}"
        ...
        <input id="phone" type="tel" name="phone" placeholder="{{ $currentCountry->mask }}"
            class="form-control js-phone-input @error('phone') is-invalid @enderror"
            value="{{ old('phone', optional(auth()->user())->phone) }}" data-code="{{ $currentCountry->code }}"
            {{ $readonly ? 'readonly' : null }} {{-- @readonly($readonly) --}}>
        @error('phone')
