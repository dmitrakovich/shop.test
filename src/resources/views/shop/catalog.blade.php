    ...
    <div class="col-12">
        <div class="row d-flex d-lg-none mb-2">
            <div class="col-6 align-self-center">
                <button class="btn btn-outline-dark rounded" type="button" data-toggle="collapse"
                    data-target="#sidebarFilters">
                    Фильтр
                </button>
        ...
        @if (!empty($badges))
            <div class="d-flex flex-wrap my-3">
                @foreach ($badges as $badge)
                    <a href="{{ $badge->url ?? '/catalog' }}"
                        class="border py-2 px-4 m-1 d-inline-flex alight-items-center">
                        {{ $badge->name }}<span class="ml-1">@include('svg.close')</span>
                    </a>
                @endforeach
            </div>
        @endif
