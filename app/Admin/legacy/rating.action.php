...
$select = "id, price, 100*(old_price - price)/old_price AS discount,
    ...
    0 as action, -- IF(pr.extra_field_16 = 1,100,0) AS action,
    0 as hit, -- IF(pr.label_id = 1,100,0) AS hit,
    0 as sale, -- IF(pr.label_id = 2,100,0) AS sale,
...
if (!isset($_POST['act'])) {
	$strToFile = '<?php'.PHP_EOL.'return '.var_export($ratingConfig,true).';'.PHP_EOL.'?>';
